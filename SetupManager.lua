...
function NSI:SortGroup(Flex, default, odds) -- default == tank, melee, ranged, healer
    ...
    for i=1, 40 do
        ...
        local specid = SAP_API:GetSpecs(unit) or 0
...
function NSI:ArrangeGroups(firstcall)
    ...
    if SAP.Groups.ProcessStart and now > SAP.Groups.ProcessStart+10 then SAP.Groups.Processing = false return end -- backup stop if it takes super long we're probably in a loop somehow
    ...
    for i=1, 40 do -- position in table is where the player should end up in, v.index is their current position.
        local v = SAP.Groups.units[i]
        if SAP.Groups.Processed >= SAP.Groups.total then SAP.Groups.Processing = false break end
