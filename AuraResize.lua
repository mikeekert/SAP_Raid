...
function SAP_API:AnchorSettings(type) -- call this when someone edits anchors to fix options preview
    local auraname = SAP.auranames[type]
    local groupname = auraname.." Group"
    local groupData = WeakAuras.GetData(groupname)
    local auraData = WeakAuras.GetData(auraname)
    SAP.groupData[groupname] = groupData
    SAP.auraData[auraname] = auraData
end


function SAP_API:AuraPosition(type, pos, reg)
    local auraname = SAP.auranames[type].." Group"
    local anchorData = SAP.groupData[auraname] or WeakAuras.GetData(auraname)
    if anchorData then
        if type ~= "Circle" then
           ...
           --[[ if WeakAuras.IsOptionsOpen() then
            ...
            for i =1, max do
                ...
                if reg[i].region.regionType == "text" then
                    height = SAP.AuraSizeData[type]+space or reg[i].region.height+space
                else
...
function SAP_API:AuraResize(type, positions, regions)
    local auraname = SAP.auranames[type]
    local groupname = auraname.." Group"
    local groupData = SAP.groupData[groupname] or WeakAuras.GetData(groupname)
    local auraData = SAP.auraData[auraname] or WeakAuras.GetData(auraname)
    SAP.groupData[groupname] = groupData
    SAP.auraData[auraname] = auraData
    ...
    for _, regionData in ipairs(regions) do   
        ...
        elseif region.regionType == "text" then
            ...
            SAP.AuraSizeData[type] = data.fontSize -- somehow even when setting the height it doesn't update to that value so I'm storing it here instead
